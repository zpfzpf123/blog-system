...
const palette = ['#1d4ed8', '#f97316', '#06b6d4', '#22c55e', '#8b5cf6', '#ef4444']
const densityPalette = ['#94a3b8', '#38bdf8', '#2563eb', '#f97316']
const depthPalette = ['#67e8f9', '#38bdf8', '#1d4ed8', '#1e3a8a']
const recencyPalette = ['#22c55e', '#06b6d4', '#f97316', '#64748b']
...
const maxTagCount = computed(() => Math.max(...topTags.value.map((item) => item.count), 1))
  ...
  const step = (time: number) => {
    ...
    animatedSummary.value = {
      ...
      categorizedRate: Number(
        (start.categorizedRate + (target.categorizedRate - start.categorizedRate) * eased).toFixed(1),
      ),
      recent30dPosts: Math.round(
        start.recent30dPosts + (target.recent30dPosts - start.recent30dPosts) * eased,
      ),
      avgReadingMinutes: Number(
        (start.avgReadingMinutes + (target.avgReadingMinutes - start.avgReadingMinutes) * eased).toFixed(1),
      ),
    }
    ...
    <div class="stats-shell">
      ...
      <section v-else class="chart-grid">
        <article class="chart-card trend-card">
          ...
          <svg class="trend-svg" :viewBox="`0 0 ${TREND_WIDTH} ${TREND_HEIGHT}`" preserveAspectRatio="none">
            <defs>
            ...
            <polyline class="trend-line" :points="trendPath"></polyline>

            <g v-for="point in trendPoints" :key="point.label">
              <circle class="trend-point" :cx="point.x" :cy="point.y" r="5" />
              <text class="trend-point-value" :x="point.x" :y="point.y - 10">{{ point.count }}</text>
            </g>
          </svg>

          <div class="trend-label-row">
            <span v-for="point in trendPoints" :key="point.label" class="trend-month">
              {{ point.label.slice(5) }}
            </span>
          </div>
        </article>
          ...
          <div class="donut-content">
            <div class="donut-shell">
              ...
              <div class="donut-center">
                <strong>{{ formatInteger(stats.summary.totalPosts) }}</strong>
                <span>篇文章</span>
              </div>
            </div>

            <ul class="legend-list">
              <li v-for="segment in donutSegments" :key="`legend-${segment.name}`" class="legend-item">
        ...
        <article class="chart-card small-card">
          <header class="chart-header">
            <h3>标签热度 Top 8</h3>
            <span>主题曝光</span>
          </header>
          <div class="bars-list">
            <article v-for="(tag, index) in topTags" :key="tag.name" class="bars-row" :style="{ '--delay': `${index * 60}ms` }">
              <div class="bars-head">
                <span>{{ tag.name }}</span>
                <strong>{{ tag.count }}</strong>
              </div>
              <div class="bars-track">
                <span class="bars-fill" :style="{ width: `${(tag.count / maxTagCount) * 100}%` }"></span>
              </div>
            </article>
          </div>
        </article>
        ...
        <article class="chart-card small-card">
          ...
          <div class="bars-list">
            <article v-for="bar in tagDensityBars" :key="bar.label" class="bars-row">
              <div class="bars-head">
                <span>{{ bar.label }}</span>
                <strong>{{ bar.count }}</strong>
              </div>
              <div class="bars-track">
                <span class="bars-fill" :style="{ width: `${bar.ratio * 100}%`, backgroundColor: bar.color }"></span>
              </div>
            </article>
          </div>
        </article>
        ...
        <article class="chart-card small-card">
          ...
          <div class="depth-columns">
            <article v-for="bar in contentDepthBars" :key="bar.label" class="depth-column">
              <span class="depth-count">{{ bar.count }}</span>
              <div class="depth-track">
                <span class="depth-fill" :style="{ height: `${Math.max(bar.ratio * 100, 5)}%`, backgroundColor: bar.color }"></span>
              </div>
        ...
        <article class="chart-card small-card">
          ...
          <div class="bars-list">
            <article v-for="bar in recencyBars" :key="bar.label" class="bars-row">
              <div class="bars-head">
                <span>{{ bar.label }}</span>
        ...
        <article class="chart-card small-card">
          ...
          <div class="hour-columns">
            <article v-for="bar in hourBars" :key="bar.label" class="hour-column">
              <span class="hour-count">{{ bar.count }}</span>
              <div class="hour-track">
                <span class="hour-fill" :style="{ height: `${Math.max(bar.ratio * 100, 5)}%`, backgroundColor: bar.color }"></span>
              </div>
...
.metric-glow {
  ...
  top: -60px;
  border-radius: 999px;
  background: radial-gradient(circle, color-mix(in srgb, var(--accent), transparent 64%) 0%, transparent 70%);
}
...
.loading-line {
  height: 16px;
  border-radius: 999px;
  background: linear-gradient(90deg, rgba(203, 213, 225, 0.3) 0%, rgba(148, 163, 184, 0.44) 45%, rgba(203, 213, 225, 0.3) 100%);
